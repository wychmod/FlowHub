...
import { Alert, App as AntdApp, Button, Form, Space, Table, Tag, Typography } from 'antd';
...
export function OrderListPage({ onNavigate }: OrderListPageProps) {
  ...
  // 最近成功数据兜底：统一覆盖 pending（平滑换页）与 error（保留上次结果，F18）两种 data 缺失场景
  ...
  useEffect(() => {
    if (!data || sort == null) return; // null 为默认态（≡ DEFAULT_SORT），无需校正
