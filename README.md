...
当前仓库为按 [docs/prd.md](docs/prd.md)、[docs/be-td.md](docs/be-td.md)、[docs/fe-td.md](docs/fe-td.md) 搭建的教学/演示项目：订单条件查询（后端接口 + 前端完整列表页，含筛选、排序、勾选与导出入口）与导出消息链路（创建受理 → Outbox 可靠投递 → RabbitMQ 消费端条件抢占与 Attempt 审计 → 执行体按任务快照 Keyset 分批读取订单 → SXSSF 流式写 Excel → 进度状态分层通知（MySQL 事实源 + Redis 投影 + SSE 广播 + HTTP 校准）→ 文件发布协议（临时文件原子移动发布 + 成功终态登记 + 受控下载））已实现，过期清理与崩溃恢复在后续迭代实现。
...
2. 导出任务闭环补全：文件发布与成功终态、下载接口已实现（见 [docs/export-file-publishing-notes.md](docs/export-file-publishing-notes.md)）；后续为过期清理与崩溃恢复（第 19 章：候选扫描、孤儿文件回收、RUNNING 僵尸任务恢复、SUCCEEDED → EXPIRED 推进）、详情/重试接口（be-td.md 4.6-4.10、6-9；创建接口、Job/Outbox 同事务落库与 RabbitMQ 消费端条件抢占已就绪，消费链路见 [docs/export-consumer-claim-attempt-notes.md](docs/export-consumer-claim-attempt-notes.md)）。
