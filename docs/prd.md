# ExportFlow 企业级异步导出中心 - 产品需求文档（PRD）
...
ExportFlow 提供一个独立、可复用的异步导出闭环：用户筛选或勾选订单，选择导出字段后创建任务；服务在后台分批读取数据并流式生成 Excel；前端持续展示任务进度，完成后下载文件。
...
- 命中数量超过 100000 行时不创建任务，提示缩小筛选范围。
