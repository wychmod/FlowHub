...
FlowHub 是一个企业级异步 Excel 导出中心的教学/演示项目。订单查询能力（条件筛选 + 排序）已实现并接入真实 MyBatis + MySQL 持久化；订单列表页前端（筛选 + 排序 + 勾选 + 导出入口）已实现；导出创建接口（`POST /api/v1/export-jobs`，DTO 校验 → Command 规范化 → 业务校验 → 幂等 → 同事务写 export_jobs/outbox_events → 202 受理）已实现；Outbox 可靠投递管道（`export/mq/`：分发器定时扫描 + Publisher Confirm 闭环 + 至少一次投递）与消费端（手动 Ack + CAS 条件抢占 + Attempt 审计：重复投递收敛为最多一次有效执行）已实现；执行体已落地确定性数据读取管道（任务快照重建 + ID 高水位 + Keyset 游标批查）；进度状态分层与实时通知已落地（MySQL 条件推进事实源 + Redis 可失败投影 + SSE AFTER_COMMIT 广播 + HTTP 校准）；SXSSF 流式 Excel 生成已落地（滑动窗口 + 双临时文件隔离 + safeText 防注入）；文件发布协议与安全下载已落地（受控 exportRoot + 路径双层防腐 + ATOMIC_MOVE 原子发布 + markSucceeded 成功终态 + 数据库失败补偿删除 + 下载接口）；恢复与清理已落地（启动恢复收敛租约失效的 RUNNING + 人工重试 + 过期文件「删除成功才 EXPIRED」+ 孤儿文件三维对账）。导出进度 SSE 前端消费（混合实时状态同步：SSE 通知 + `job_version` 版本栅栏 + HTTP 校准 + 轮询降级）已随 `useExportEvents` Hook 与导出任务页交付。订单 Excel 导入功能（模板下载 + 文件/结构/行级三层校验 + 异步导入任务复刻 Outbox/消费/进度/SSE 管道 + 错误报告下载 + PARTIAL 部分成功语义 + 前端导入入口）：**后端已完整实现**（包 `orderimport/`，含接受链、SAX 流式读、冲突预查、错误报告、维护清理与集成测试），导入格式锚定为导出全 9 列格式（导入导出互逆）；前端入口为独立「导入任务」页，已实现。
...
- 导出任务列表接口：http://localhost:8080/api/v1/export-jobs（真实分页查询，可选 `status` 过滤，返回进度/下载/错误等完整派生字段）
- 导出任务创建接口：`POST /api/v1/export-jobs`（需 `Idempotency-Key` 头与 JSON 请求体）
...
6. **注释精简，只说签名看不出来的事**：类 javadoc 1-3 行说明职责即可；方法 javadoc 原则上 1 行说明功能；`@param`/`@return` 仅在参数含义、取值约束或返回值语义无法从签名自明时才写，且每个 1 行以内。**禁止**：长篇复述设计文档内容、解释历史改动过程、为方法体只有一两行的简单方法写多行 javadoc。仅当存在签名无法表达的**关键设计决策或边界约束**（如「BigDecimal 必须用 compareTo」「NULL 视为最小值与 MySQL 对齐」）时才额外说明。排版基准示例：`common/web/param/ParamUtils.java`。
...
- `App.tsx`：根组件，目前用本地状态切换三个页面（订单 / 导出任务 / 导入任务，尚未引入路由），向订单页传入 `onNavigate`（创建成功后「查看任务」跳转导出任务页）。
...
- 导出任务创建接口（`POST /api/v1/export-jobs`：DTO 跨字段校验 → `CreateExportJobCommand` 规范化 → 存在性/命中数业务校验 → 幂等键 + request hash 判重（复用/409 冲突）→ 同事务写 `export_jobs`(PENDING) 与 `outbox_events` → 202 受理；筛选命中上限 `export.filter-max-rows` 可配置，默认 500000）。
- Outbox 可靠投递管道（`export/mq/`：`RabbitConfig` 拓扑 + `OutboxDispatcher` 定时扫描发布 + `ExportJobMessage` 最小契约消息；Confirm ACK 且无 Returned 才回填 `published_at`，失败/NACK/退回/超时保留事件下轮补发并记 `outbox_publish_deferred` 日志；`spring.rabbitmq` 配置 `publisher-confirm-type: correlated` + `publisher-returns` + `template.mandatory: true`）。
- RabbitMQ 消费端（`ExportJobConsumer` 手动 Ack 消费 + `ExportJobService.claimPendingJob/markFailed` CAS 条件抢占与失败收敛 + `ExportJobAttemptMapper` Attempt 审计 + `ExportExecutionService` 执行壳：重复投递收敛为最多一次有效执行，抢占与 Attempt 同事务原子，契约不支持 Reject 转 DLQ，执行失败收敛 Job/Attempt FAILED；`listener.simple` 配 manual/prefetch=1/concurrency=2，集成测试 `ExportJobConsumerTest`）。
- 确定性数据读取管道（创建时 `ExportOrderMapper.snapshotByCriteria` 单查询统计命中 COUNT 与范围内 MAX(id) 高水位；执行体 `ExportExecutionService.runJob` 按任务快照 Keyset 批量读取——`findBatch` 复用 `criteriaConditions` 共享片段以 `ORDER BY id ASC LIMIT` 恒定成本推进，每批落 `processed_rows`，空批/不足一批双结束；`ExportExecutionIntegrationTest` 7 用例覆盖高水位阻断/批次边界/快照重建/排除 ID/空值防御/round-trip）。
- 进度状态分层与实时通知（条件 UPDATE 进度守卫（`processed_rows` 单调 + 终态单向 + heartbeat/lease 续期，0 行 fail-fast）+ `ExportJobChanged` 应用事件 + AFTER_COMMIT SSE 广播（4 类事件、`jobId:version` 事件 id、15s 心跳、坏连接隔离、发送前重读 Job）+ Redis Hash 投影（TTL 48h、失败降级、99% 封顶）+ 回滚事务不广播；集成测试 `ExportProgressSseIntegrationTest` 8 用例与 `ExportJobEventPayloadTest` 6 用例）。
- SXSSF 流式 Excel 生成（`ExcelExportWriter` + `WorkbookSession`——窗口 100 滑动 + 压缩临时文件 + 列白名单二次复核 + safeText 公式注入防护 + 金额 NUMERIC/样式单例 + close 链 write→close→dispose（suppressed exception）；`ExportFileService` 受控分配临时文件、失败路径删除半成品；Writer 单测 6 用例以真实 XSSF 重开断言 + 执行体扩展 2 用例（行数一致/写失败不虚假推进+清理））。
- 文件发布协议与安全下载（`ExportFileService` 受控文件边界——exportRoot 提纯、层级目录 `<UTC 日期>/<jobId>/attempt-N`、路径双层防腐（normalize 拒 `..` 与根组件 + 逐段符号链接/toRealPath 验真）；执行体「写完 .tmp → publish 原子移动发布 → markSucceeded 事务登记 → DB 失败补偿删除」收敛顺序，`AtomicMoveNotSupportedException` 不降级；`ExportJobService.markSucceeded` 同事务置 Job/Attempt SUCCEEDED 并回填 file_path/file_size_bytes/finished_at/expired_at；下载接口 `GET /{job_id}/download` 只按 Job 查（SUCCEEDED 且未过期 → resolvePersisted 受控解析 → 文件流）；`ExportFileServiceTest` 8 用例 + `ExportJobDownloadControllerTest` 7 用例 + 执行体发布/补偿用例）。
- 恢复与清理（`ExportMaintenanceService` 启动恢复只收敛「租约已失效」的 RUNNING 为 FAILED(SERVICE_RESTARTED)（Attempt 先行、Job 收尾、不自动重跑）；`POST /{job_id}/retry` 人工重试（FAILED→PENDING 条件重置 + 同事务新 Outbox，失败 Attempt 证据保留，`EXPORT_JOB_NOT_RETRYABLE` 409）；过期清理「文件删除成功才 markExpired EXPIRED」（路径非法/删除失败保持 SUCCEEDED 下轮再试，条件更新 0 行不发事件）+ Redis 投影删除；孤儿文件三维对账（宽限期 1h + 活跃租约 `countRunningWithValidLease` + Job/Attempt 引用检查）；`ExportMaintenanceIntegrationTest` 6 用例（活跃租约不误伤/重试保留历史+新 Outbox+抢占递增/上限拒绝/删除即事实/孤儿 tmp 活跃保护/xlsx 三态））。
...
- 订单列表接口（真实 MyBatis + MySQL 持久化、服务端分页 + 条件筛选 + 排序白名单与 sort_by/sort_order 回显；含内存/MyBatis 行为对齐测试）。
- 导出任务列表接口（真实分页查询 + 可选 `status` 过滤 + 派生字段 progress_percent/downloadable；Maven 侧 `findPage`/`countAll` SQL 在 `ExportJobMapper.xml`，集成测试 `ExportJobListTest` 9 用例覆盖空表/倒序分页/状态过滤/派生计算/错误字段透出，并发幂等由 `ExportJobIdempotencyConcurrencyTest` 2 用例验证唯一约束裁决）。
- 前端订单列表页（react-query + antd Table 服务端分页）：8 项条件筛选（草稿/已提交严格分离，输入不触发请求）、订单号/金额/下单时间三列表头三态排序（以响应回显对齐）、跨页勾选（上限 1000 条，超限整体拒绝）、「导出已选 / 导出筛选结果」入口与配置弹窗（9 列白名单、文件名、幂等键）；查询失败保留上次数据与用户意图。
...
- 前端 API 防腐层：`requestJson` 合法 Envelope 结构校验（2xx 非 Envelope 抛 `Invalid API envelope`）与统一错误转换（`ApiError` 携带 message/code/status/traceId/fieldErrors）；文件下载协议工具 `api/download.ts`（`parseBlobError`/`filenameFromDisposition`/`saveBlob`）与业务语言下载接口 `downloadExportJob`。
- 前端导出任务页：`useExportEvents` Hook（SSE 连接状态机 + 版本栅栏 + 乐观局部更新 + 失效收敛 + 降级轮询 + 生命周期清理，jsdom 单测 6 用例）+ `ExportJobsPage` 完整页面（状态 Tag/进度条/文件大小/下载/重试/分页/ConnectionBadge）+ `api/exportApi.ts` 补全 `retryExportJob`/`exportEventsUrl` 与完整列表类型；后端列表接口联动（`findPage`/`countAll`/派生字段）；已做真实后端端到端联调（创建 5 万行任务 → 执行推进 → SUCCEEDED → 下载 1.6MB Excel → 重试 409）。

- 订单 Excel 导入后端（完整落地：Flyway V9 建表、模板/ExcelImportReader SAX/acceptance 三层校验/Outbox+消费+进度+SSE 全链路/PARTIAL 错误报告/维护清理与集成测试（含 `ImportProgressSseIntegrationTest` 进度守卫/AFTER_COMMIT/坏连接隔离矩阵））。

当前尚未实现：
- 导出任务闭环补全（文件发布、成功终态、下载接口、崩溃恢复、人工重试与过期清理均已落地；后续为任务详情接口与架构复盘（多实例部署/对象存储/集群调度））。
- Redis（幂等缓存与状态缓存；进度投影缓存已落地）。
- 导出任务详情接口（任务详情未实现；列表真实化、SSE 事件、下载与重试均已落地，前端导出页与 `useExportEvents` 已完成端到端联调）。
...
- 上传受理的 multipart 防护（`common/web/upload/OversizeRequestBodyFilter`）：`spring.servlet.multipart.max-file-size`（默认 11MB）/`max-request-size`（12MB）——业务层自校验 ≤10MB（`IMPORT_FILE_TOO_LARGE`），multipart 闸略放大让请求到达业务校验；超过 max-file-size 的请求体由该 Filter 按 `Content-Length` 预拒（先读光请求体再回 400 `VALIDATION_ERROR`，避免容器中止 multipart 解析关闭连接导致客户端「写不完 body 也收不到响应」的无限挂起，chunked 等无长度场景仍由容器闸兜底并经 `GlobalExceptionHandler` 的 `MaxUploadSizeExceededException`/`MultipartException` 转 400）；配套 `server.tomcat.max-swallow-size: -1`。Filter 单测 `OversizeRequestBodyFilterTest`、真实 Tomcat 契约测试 `ImportJobUploadControllerTest`（手工拼 multipart body 携带精确 Content-Length，对齐浏览器 fetch FormData 行为）。
