...
- SXSSF 流式 Excel 生成（第 17 章：`ExcelExportWriter` + `WorkbookSession`——窗口 100 滑动 + 压缩临时文件 + 列白名单二次复核 + safeText 公式注入防护 + 金额 NUMERIC/样式单例 + close 链 write→close→dispose（suppressed exception）；`ExportFileService` 分配 `job-{id}-attempt-{n}.tmp`、失败路径删除半成品；Writer 单测 6 用例以真实 XSSF 重开断言 + 执行体扩展 2 用例（行数一致/写失败不虚假推进+清理），见 `docs/export-sxssf-writer-notes.md`）。
...
设计文档中规划但尚未实现：
- 导出执行器收尾（数据读取、SXSSF 写盘、进度通知均已就绪：执行体写盘完成后仍以「成功终态尚未实现」收敛 FAILED 并清理临时文件；后续按第 18 章接入文件发布（原子移动 `job-{id}-attempt-{n}.tmp` → 正式文件）、`markSucceeded` 成功终态与 `file_path`/`file_size_bytes` 回填；`OrderCriteria` 快照重放与 `ids` 精确取数的查询契约已就绪，见 `docs/order-query-design.md` 第八节第 9 步）。
- Redis（幂等缓存与状态缓存；进度投影缓存已随第 16 章落地）。
- 导出任务详情/重试、下载与文件过期清理（SSE 事件接口已随第 16 章落地；前端 `useExportEvents` 消费为独立交付计划，见 `docs/export-sse-design.md`，端到端联调依赖任务列表接口真实化 P-4）。
...
- 导出执行的业务配置：`export.execution.batch-size`（执行体 Keyset 批查每批行数，默认 1000）、`export.execution.lease-minutes`（进度推进时 heartbeat/lease 续期时长，默认 5）与 `export.files.dir`（业务临时文件目录，默认 `export-files/`，已被 `.gitignore` 忽略），均走 `@Value` 默认值不落 yml；POI SXSSF 窗口为 `ExcelExportWriter.ROW_WINDOW = 100` 常量。
