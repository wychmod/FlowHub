...
- 导出执行的业务配置：`export.execution.batch-size`（执行体 Keyset 批查每批行数，默认 1000）与 `export.execution.lease-minutes`（进度推进时 heartbeat/lease 续期时长，默认 5），均走 `@Value` 默认值不落 yml。
