...
设计文档中规划但尚未实现：
- 导出执行器本体（消费链路已就绪：Consumer 抢占、Attempt 审计与失败收敛已实现，`ExportExecutionService` 执行体为「尚未实现」占位失败收敛；后续按第 15 章实现游标批量读取 + POI SXSSF 流式写 Excel 至 `export-files/`；`OrderCriteria` 快照重放与 `ids` 精确取数的查询契约已就绪，见 `docs/order-query-design.md` 第八节第 9 步）。
