...
- 导出任务列表接口：http://localhost:8080/api/v1/export-jobs（真实分页查询，返回进度/下载/错误等完整派生字段，见 be-td.md 4.6）
...
- 导出任务列表接口（真实分页查询 + 派生字段 progress_percent/downloadable，契约 be-td.md 4.6；Maven 侧 `findPage`/`countAll` SQL 在 `ExportJobMapper.xml`，集成测试 `ExportJobListTest` 6 用例覆盖空表/倒序分页/派生计算/错误字段透出）。
