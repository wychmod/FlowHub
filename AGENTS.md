...
设计文档中规划但尚未实现：
- 导出任务闭环补全（文件发布、成功终态与下载接口均已随第 18 章落地，见 `docs/export-file-publishing-notes.md`；后续按第 19 章接入过期清理与崩溃恢复——候选扫描（temporary/finalCandidatesOlderThan）、孤儿文件回收、RUNNING 僵尸任务恢复（lease 已在第 14 章预置）与 SUCCEEDED → EXPIRED 推进；`OrderCriteria` 快照重放与 `ids` 精确取数的查询契约已就绪，见 `docs/order-query-design.md` 第八节第 9 步）。
- Redis（幂等缓存与状态缓存；进度投影缓存已随第 16 章落地）。
- 导出任务详情/重试与文件过期清理（第 19 章；SSE 事件接口已随第 16 章落地，下载接口已随第 18 章落地；前端 `useExportEvents` 消费为独立交付计划，见 `docs/export-sse-design.md`，端到端联调依赖任务列表接口真实化 P-4）。
