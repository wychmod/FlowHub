...
设计文档中规划但尚未实现：
- 导出任务闭环补全（文件发布、成功终态、下载接口、崩溃恢复、人工重试与过期清理均已随第 18/19 章落地，见 `docs/export-file-publishing-notes.md` 与 `docs/export-recovery-cleanup-notes.md`；后续为导出任务列表接口真实化（P-4）与详情接口、第 20 章架构复盘（多实例部署/对象存储/集群调度）；`OrderCriteria` 快照重放与 `ids` 精确取数的查询契约已就绪，见 `docs/order-query-design.md` 第八节第 9 步）。
- Redis（幂等缓存与状态缓存；进度投影缓存已随第 16 章落地）。
- 导出任务详情与列表接口真实化（任务详情接口未实现；SSE 事件接口已随第 16 章落地，下载与重试接口已随第 18/19 章落地；前端导出任务页与 `useExportEvents` 消费为独立交付计划，见 `docs/export-sse-design.md`，端到端联调依赖任务列表接口真实化 P-4）。
